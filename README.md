...
まず `metaphor` コマンドを入れます。これは新しいスケッチ作成、実行、更新を扱うためのCLIです。

```bash
brew install shinyaoguri/tap/metaphor
```

Homebrewを使わない場合は、GitHub Release のバイナリを直接インストールできます。

```bash
curl -fsSL https://raw.githubusercontent.com/shinyaoguri/metaphor-cli/main/scripts/install.sh | bash
```

direct installer を使った場合、`~/.local/bin` が `PATH` に入っていなければ追加してください。
...
以前に direct installer で入れた `~/.local/bin/metaphor` が残っている場合は、
Homebrew 版より先に実行されることがあります。Homebrew 版を使う場合は削除してください。
...
brew upgrade metaphor    # Homebrewで入れたCLI本体を更新
