...
実際に判定に困った 3 件 (Issue #663 の調査で見つかった、タイトルに型が無い
Issue) も pin してある — 写像を緩めて triage を減らそうとしたときに、この 3 件が
誤ったラベルに落ちることで気づけるように。
"""
...
class LabelForTitle(unittest.TestCase):
    ...
    def test_unknown_type_goes_to_triage(self):
        """`style` / `revert` は PR 側でも採っていない。黙って maintenance に寄せない。"""
        self.assertEqual(li.label_for_title("style(Core): 整形"), TRIAGE)
        self.assertEqual(li.label_for_title("revert: 戻す"), TRIAGE)

    def test_freeform_titles_go_to_triage(self):
        """判定できなかった実例 (Issue #663 の調査時点) を pin する。"""
        for title in (
            "changelog.d のファイル名検証を per-PR CI でも走らせる(現在は release 時のみ)",
            "MLTextureConverter.texture(from: CVPixelBuffer) が IOSurface をリークする（CVMetalTextureCache が flush されない）",
            "共有セッション: 人間の watch ＋ AI の mcp が同一スケッチを協調 (Phase 1: 観測アタッチ)",
        ):
    ...
    def test_real_titles(self):
        ...
        cases = {
            "fix(Examples): SaveFile2 が background() を呼ばず、打った点が黒地に黒で見えない": "type: bug",
            "design(Probe): ユーザーコードを汚さない on-demand Render Trace を検討する": "type: design",
            "api(Color): 初期化子のアルファのラベルが `alpha:` と `a:` で揺れている": "type: feature",
            "test(golden): 3D ライティングのゴールデンが鏡面ハイライトを写していない": "type: maintenance",
            "docs(tutorial): motion.json の width が kind: sheet に効かない": "type: docs",
        }
...
class SyncLabelsDryRun(unittest.TestCase):
    ...
    BEFORE = [
        ...
        "release:patch", "no-changelog", "dependencies",
    ]
