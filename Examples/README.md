...
- **順番に学ぶ** → [LEARNING_PATH.md](LEARNING_PATH.md)（英語）。278 本の索引ではなく、難度タグを使ったカテゴリ別の推奨順路（代表 3〜5 本 x 各カテゴリ）です。
...
- **`probe-input.jsonl`**（同、JSON Lines の入力台本）— マウス・キーボードの入力が無いと題意が出ないもの。入力を流してから撮る経路はまだ無く（[#610](https://github.com/shinyaoguri/metaphor/issues/610)）、いまは撮影対象から外れます
