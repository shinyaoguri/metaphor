...
final class Convolution: Sketch {
    var config: SketchConfig {
        SketchConfig(title: "Convolution", width: 640, height: 360)
    }
    ...
    func setup() {
        noLoop()
        img = generateTestImage(Int(width), Int(height))
    }
    ...
    func convolve(_ x: Int, _ y: Int, _ kernel: [[Float]]) -> (UInt8, UInt8, UInt8) {
        ...
        for i in 0..<3 {
            for j in 0..<3 {
                let xloc = max(0, min(img.width - 1, x + i - 1))
                let yloc = max(0, min(img.height - 1, y + j - 1))
                let loc = (yloc * img.width + xloc) * 4
    ...
    func draw() {
        ...
        let xend = min(img.width, Int(mouseX) + regionW / 2)
        let yend = min(img.height, Int(mouseY) + regionW / 2)
