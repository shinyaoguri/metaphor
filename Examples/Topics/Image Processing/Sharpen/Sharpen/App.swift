    ...
    func draw() {
        ...
        for y in 1..<(h - 1) {
            for x in 1..<(w - 1) {
                ...
                let outIdx = (y * w + x) * 4
                sharpImg.pixels[outIdx] = UInt8(max(0, min(255, sumR)))
                sharpImg.pixels[outIdx + 1] = UInt8(max(0, min(255, sumG)))
                sharpImg.pixels[outIdx + 2] = UInt8(max(0, min(255, sumB)))
                sharpImg.pixels[outIdx + 3] = 255
            }
        }
        sharpImg.updatePixels()
        image(sharpImg, width / 2, 0)
    }
}
