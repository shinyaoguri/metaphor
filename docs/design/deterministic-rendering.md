...
- **ステータス**: 実装完了（2026-06-30。PR #107 / #108 マージ済み、ADR-0002 Accepted）
...
### 即時経路に残る重ね順の条件（2026-08-16 追記・#682）

上の「実装完了」で解消したのは **記録経路**（影オン / `METAPHOR_COMMAND_RECORD=1`）の話で、**即時経路（影オフ既定）には呼び出し順が保たれない条件が残る**。即時経路の重ね順は「呼び出し順」ではなく「エンコードされた順」で決まり、2D はバッチが途中で吐かれた時点でそこまでが確定するため、**3D と重なる 2D が途中で吐かれると 3D の背後へ回る**。

したがって即時経路で呼び出し順どおりの合成が保証されるのは、**3D と重なる 2D がフレーム末尾まで吐かれない最後のバッチに収まっている**ときに限られる。バッチが途中で吐かれる契機は `blendMode()` の変更・`image()` / `text()` との交互・`circles()` 等のインスタンス描画との交互・`beginClip()` / `endClip()`・頂点バッファの上限到達の 5 系統で、**「3D の前面に置いた 2D の後にテキストを 1 行足しただけで背後に回る」**という形で現れる（実測では 10.7% の画素が記録 ON/OFF で食い違った）。

契機の一覧・実測値・記録経路との対応は [ADR-0003](../adr/0003-unified-command-stream.md) の Amendment 内「2026-08-16 追記（#682）」が正典。上の「非目標 / 互換性の境界（既知の制限）」は #70 時点の記述で、その 1 つ目（2D を 3D の背後に描いた場合）が #71 で解消したのは記録経路に限られる点に注意。
