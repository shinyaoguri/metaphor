...
- **ステータス**: 運用中(living document)。レビュー所見は確定、実装計画は 2026-08-01 にユーザーと戦略合意済み(v0.9.x 育成方式・既知課題は v0.8.x で完遂)。**v0.9.0 は 2026-08-10 にリリース済み。ただし同リリースで宣言した API 凍結は 2026-08-13 に撤回した**([ADR-0009](../adr/0009-unfreeze-api-until-1-0.md)) — 1.0.0 までは設計の質を優先し、breaking change を minor で許容する
...
| 期間 | 位置づけ | 内容 |
|---|---|---|
| **v0.8.x(完了)** | 壊してよい最後の期間 | 本ドキュメントの既知課題(G1〜G21・A 節)を**すべてここで片付ける**。breaking change はこの期間に完遂。Phase 2 機能開発(Parameter Store 等)と並行 |
| **v0.9.0(2026-08-10 リリース済み)** | 当初は API 凍結宣言 → **2026-08-13 に撤回**(ADR-0009) | 凍結の入口条件を満たさないまま出た(#477)ため、凍結を解いて設計作業を続ける判断に切り替えた |
| **v0.9.x(現在)** | 1.0 前の最後の設計期間 | **設計が正当化するなら breaking を minor で入れてよい**(deprecation 窓が既定・`### Breaking Changes` + 移行表は必須)。英語化の継続、実運用(metaphor-sketches)フィードバック。**コミュニティ条件(下記)を満たすまで期間の上限は設けず、改良を継続する** |
| **v1.0.0** | 昇格 = 凍結の開始点 | 昇格条件(後述)= 技術条件 + コミュニティ条件(**GitHub スター 100 超**)。ここから public API の breaking には major が要る(2026-08-02 決定 + ADR-0009) |

**凍結範囲(論点 2)の決着**: 全 13 モジュールのうちどれを 1.0 で凍結し、どれを preview 宣言(breaking は minor で可)のまま出すかは、**昇格時点の実績から線を引く**。0.9.x を凍結扱いで運用して判定する当初案は、凍結を撤回したため成立しない — 判定材料は「0.9.x で壊さずに済んだか」ではなく「昇格時点でそのモジュールの設計が落ち着いているか」になる。
...
2. **API doc コメント英語化(G9)** — 本 Epic #314 の W3-1(#334)。最大工数だが `llms.txt` は生成物なので自動追随する(= AI 向け正典も同時に英語化される)。中核 API(`circle`/`rect`/`fill`)ほど日本語という現状の逆転を優先的に解消
...
| ID | 内容 | 対応 | 規模 |
|---|---|---|---|
| W3-1 | **API doc コメント英語化**(#334。ロードマップ Epic I の I2 として起票され、Epic #295 のクローズに伴い本 Epic へ一本化)。モジュール単位に分割し、周辺モジュール(Audio/Network/Physics 等)から先行、**Core は W1-3 命名統一の後**。`llms.txt` は生成物なので自動追随。機械的作業のためサブエージェント委譲向き | G9 | **L** |
...
| W4-2 | 契約フリーズ点の明記(`frame.json` の全キー凍結 / stdin プロトコルの互換規約とバージョニング)。**クロスリポ同時 PR**。Parameter Store の schema 変更(Epic D の D3/D4)が入った後に実施 — **D3/D4 は完了済み(#451)でブロック解除。v1.0 技術条件へ移設**(2026-08-13) | G19 | M | Phase 2 D3/D4 |
...
2. **英語化の位置づけ**: W3-1 は additive なので 0.9.x に食い込んでよい。ただし v1.0.0 昇格条件には含める(下記)
3. **preview 宣言の要否**: 1.0 昇格時点の設計の落ち着き具合で判断(マイルストーン節参照。当初は「0.9.x の凍結実績で判断」だったが、凍結撤回により判定材料が変わった — ADR-0009)

## 完了判定

### v0.9.0 の入口条件(API 凍結宣言)— **解散**(2026-08-13)

**決着**: このチェックリストは役目を終えた。`v0.9.0` は未チェック項目を残したまま 2026-08-10 にリリースされ(#477 の指摘)、その後 **API 凍結そのものを撤回した**(ADR-0009)。凍結しない以上「凍結の入口条件」は成立しないため、リストは履歴として残し、以後の判定には使わない。

未消化だった 2 項目の行き先:

| 項目 | 行き先 |
|---|---|
| Phase 2(Parameter Store)完了・実運用 1〜2 サイクル経過 | D1〜D5 は完了済み(#451)。**実運用サイクルは v1.0 技術条件の「metaphor-sketches 実運用検証」(Epic #414)に統合** |
| W4-2(契約フリーズ点 #339)が両リポで合意済み | 依存していた Phase 2 D3/D4 は完了済みでブロック解除。**v1.0 技術条件へ移設**(下記) |

`#322` フェーズ 2(旧名削除)は「凍結前に片付ける」対象から「1.0 までに片付ける宿題」へ位置づけが変わった。凍結撤回により minor + `### Breaking Changes` で実施できる（**2026-08-14 に完了**）。

以下は当時の記録(2026-08-13 時点の状態のまま残す):
...
(当時の記述: 「残る入口条件は Phase 2 とその後続(W4-2)のみ。ほかに v0.9.0 前に片付ける必要があるものとして #322 フェーズ 2 と #388 がある」。#388 は 2026-08-02 に完了、#322 フェーズ 2 は未実施のまま `v0.9.0` が出た — これが凍結撤回の直接の引き金になった。)
...
- [ ] **既知の設計上の宿題が片付いている**(2026-08-13 差し替え — 旧条件「0.9 系で breaking なしの実績」は凍結撤回により判定材料にならなくなった。ADR-0009)。昇格時点で open な API 設計課題がゼロであること。現時点の中身 = #519・#523(Processing 互換の意味論)。片付いた分: #322 フェーズ 2(旧名削除)は 2026-08-14 に完了、#339(契約フリーズ点・旧 W4-2)は 2026-08-18 に完了(フリーズ点と `schemaVersion` bump 規則を CONTRACT.md へ明記)。#504(矩形版 `text()` の反転)は 2026-08-13 に実装バグと判明し修正済み — 意味論の選択ではなかった
...
v1.0.0 は SemVer 契約の凍結宣言であり、利用者がいて初めて意味を持つ。コミュニティ条件を満たすまでは 0.9.x のまま改良(品質向上・ドキュメント・実運用フィードバックの反映。**設計が正当化するなら breaking も含む** — ADR-0009)を継続する。スター数は外部要因のため期限は設けない。技術条件が先に揃った場合も 0.9.x を延長する — その間に設計を練り直せること自体が、凍結を先送りする利点になる。
