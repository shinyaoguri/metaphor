...
`#322` フェーズ 2(旧名削除)は「凍結前に片付ける」対象から「1.0 までに片付ける宿題」へ位置づけが変わった。凍結撤回により minor + `### Breaking Changes` で実施できる。
...
- [ ] **既知の設計上の宿題が片付いている**(2026-08-13 差し替え — 旧条件「0.9 系で breaking なしの実績」は凍結撤回により判定材料にならなくなった。ADR-0009)。昇格時点で open な API 設計課題がゼロであること。現時点の中身 = #322 フェーズ 2(旧名削除)/ #519・#523(Processing 互換の意味論)/ #339(契約フリーズ点・旧 W4-2)。#504(矩形版 `text()` の反転)は 2026-08-13 に実装バグと判明し修正済み — 意味論の選択ではなかった
