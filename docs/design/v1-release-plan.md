...
- **ステータス**: 運用中(living document)。レビュー所見は確定、実装計画は 2026-08-01 にユーザーと戦略合意済み(v0.9.x 育成方式・既知課題は v0.8.x で完遂)。**v0.9.0(API 凍結)は 2026-08-10 にリリース済み** — 現在は v0.9.x 育成・実証期間
...
| **v0.9.0(2026-08-10 リリース済み)** | API 凍結宣言 | 入口条件 = A 節全項目が「実施 or 見送り ADR 化」済み + Phase 2 完了。以降 v1.0 と同じ規律で運用開始 |
| **v0.9.x(現在)** | 育成・実証期間 | 原則 additive と fix のみ。英語化の継続、実運用(metaphor-sketches)フィードバック。breaking が必要になったら「凍結にまだ早かった」シグナルとして扱う。**コミュニティ条件(下記)を満たすまで期間の上限は設けず、改良を継続する** |
