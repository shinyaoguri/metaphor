...
> トラッキング: Epic shinyaoguri/metaphor#75 / 設計 #66 / 実装 metaphor-cli#19(M1) #20(M2) / README #91(M3)
...
当初欠けていた **(1) これらを束ねる単一インターフェイス（MCP）** は `metaphor mcp <sketch>` として
metaphor-cli に実装済み（本ドキュメントが定義した設計）。**(2) 状態の可視化**は共有セッション
（[shared-session.md](./shared-session.md)）へ展開した。以降は実装の指針として残す設計記録である。
...
| M3 ✅ | 「AIがmetaphorを駆動する」参照ループを README 最上位へ | metaphor #68 → #91 | 価値が表に出る |
