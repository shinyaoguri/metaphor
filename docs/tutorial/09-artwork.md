...
![格子状に並んだ水色の円。大きさと色がノイズで揺れ、下に S キーで 1 枚保存、R キーで連番という案内が出ている](https://i.gyazo.com/dec49dbe07710158c842ef3f966190d3.png)
...
![三つ葉の形をした輪に沿って並ぶ粒。青から紫へ色が変わり、下に V で動画・G で GIF という案内が出ている](https://i.gyazo.com/331e87a56723a34a812aaaa0162ef753.png)

![三つ葉の輪に沿った粒が、回りながら伸び縮みする](https://i.gyazo.com/25a2477dd103155e16c269e134635d56.webp)
...
![白い紙に細い横線を何十本も引いた、うねる縞模様。これを SVG として書き出している](https://i.gyazo.com/1b05815667d2bcd2472cf5c3802362ed.png)
...
![六角形を少しずつ回して重ねた、赤から青へ変わる線の模様](https://i.gyazo.com/e744a84e319144a4a84b212ea0370641.png)
...
![弧を描いて流れる粒の群れ。右上に FPS とフレーム時間・GPU 時間の HUD、左下に粒の数と経過フレームが出ている](https://i.gyazo.com/d6e186ec074eabb89a3be005dbf336b5.png)
...
FPS とフレーム時間は、コードから読む `performance.fps` / `performance.frameTimeMs` と**同じ実測値**です（直近およそ 1 秒の窓）。目で見た数字とコードが読む数字が食い違わないので、HUD で当たりを付けたあと、その値をそのまま「重くなったら粒を減らす」といった判断へ回せます。まだ測れないとき（起動直後・`noLoop()` で止めているとき）は `0` ではなく `--` と出ます。
