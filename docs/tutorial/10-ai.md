...
![格子に並んだ点。ノイズの値を map で見える大きさへ移してあり、桃色と紫の粒として散らばっている](https://i.gyazo.com/fa98dd63c6564f3d90bf8419981db486.png)
...
- [`map`](https://shinyaoguri.github.io/metaphor/reference/documentation/metaphorcore/map%28_:_:_:_:_:%29) / [`noise`](https://shinyaoguri.github.io/metaphor/reference/documentation/metaphorcore/noise%28_:_:_:%29)
- 統計（`stats`）の定義: [CONTRACT.md](https://github.com/shinyaoguri/metaphor/blob/main/CONTRACT.md)
- 同じ絵を再現する話: 第 9 部 9.4「きれいに焼き出す」

## 10.2 内部状態を申告する

![A・B・C の 3 つの点が線で結ばれ、いちばん近い 2 点 A-B がオレンジで強調されている。下に距離 229px の表示](https://i.gyazo.com/4ddbafeb87ffc44a9dc4f79ecb7f8d35.png)
...
![波形に沿って並んだ点が、左の青から右の桃色へ色を変えていく](https://i.gyazo.com/1c6a34f32b8d3bd6e4c3ad06d89451da.png)
...
metaphor は Processing の発想を持っていますが、Processing でも p5.js でもありません。文脈を渡さないまま頼むと、AI は近い記憶で書きます。`size(640, 360)` でキャンバスを開き、`PVector` で位置を持ち、`loadShader()` に GLSL を渡します。前の 2 つはこのライブラリに無く（キャンバスは `config`、ベクトルは `Vec2` / `Vec3`）、3 つ目は名前だけ同じで中身が違います（第 6 部 6.5 のとおり MSL しか受け取りません）。どれもビルドが通りません。
