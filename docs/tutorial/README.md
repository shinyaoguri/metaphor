...
- 動きが分からないと伝わらない節（イージング・パーティクル・入力）は、静止画に加えてアニメーション（WebP）か連番のコンタクトシートを置きます（後述）
...
| キー | 意味 |
|---|---|
| `kind` | `webp`（アニメーション WebP `{節}.webp`）か `sheet`（コンタクトシート `{節}.sheet.png`） |
...
`kind` を指定した節でも**代表静止画 `{節}.png` は作られます**（連続キャプチャの真ん中のフレーム）。本文の頭に 1 枚置き、その下に動きを貼る形になります。

**GIF ではなく WebP** です。同じ絵で 1 桁小さく、`![](...)` のまま GitHub でも website でも動きます。mp4 はさらに小さいものの、GitHub の Markdown がリポジトリ内の相対パスの動画を再生しないため採っていません（本文が GitHub 上でもそのまま読めることが要件）。生成には `img2webp`（`brew install webp`）が要ります。
