...
| 8.3 Syphon | 映像を他アプリへ流す、固定解像度出力（別パッケージ [metaphor-syphon](https://github.com/shinyaoguri/metaphor-syphon)） | `import MetaphorSyphon`, `plugins: [.syphon(name:)]` | metaphor-syphon の [`Examples/Samples/Syphon`](https://github.com/shinyaoguri/metaphor-syphon/tree/main/Examples/Samples/Syphon) |
...
### 撮影解像度は当面 640x360 のまま（[#1025](https://github.com/shinyaoguri/metaphor/issues/1025)）

節のスケッチは `SketchConfig(width: 640, height: 360)` で書きます。Probe が撮るのは
ポストエフェクト適用済みのオフスクリーンテクスチャなので、**撮影解像度 = スケッチの
`config` の値**です（ウィンドウの大きさにもレターボックスにも依存しません）。

この 640 は **Retina 画面には足りていません**。実測（#1025）:

| 面 | 本文の段の幅 | 画像の表示幅 | Retina 倍率 |
|---|---|---|---|
| website（viewport 1512） | 688px | 640 CSS px | **1.00x** |
| website（viewport 1024〜1279、右の目次が出ない幅） | 852〜951px | 640 CSS px | **1.00x** |
| GitHub の Markdown（viewport によらず頭打ち） | 1012px | 640 CSS px | **1.00x** |

どちらの面も画像を素の実寸で置くので、表示幅は素材の 640px そのままです。dpr 2 の画面では
2 倍に引き伸ばされて届きます。1280x720 で撮り直したものと比べると、輪郭の鋭さ（勾配 RMS）で
1.53 倍、細部（ラプラシアン分散）で 7.0 倍の差があり、**小さい文字ほど差が大きい**
（14pt 相当で 1.97 倍、大見出しで 1.31 倍）。

それでも **いまは上げません**。上げるには各節の `config` と本文中のすべての座標・`textSize` を
2 倍にするしかなく、それは教材そのものを書き換えることになるためです（`size(640, 360)` に
`textSize(40)` と書くから読める教材が、`size(1280, 720)` に `textSize(80)` と書いた別物に
なります）。構図を保ったまま画素だけ増やす手段（Processing の `pixelDensity(2)` 相当）が
ライブラリ側に無いことが根本の理由で、[#1033](https://github.com/shinyaoguri/metaphor/issues/1033)
で扱います。そこが入れば、この節の撮り直しは `make tutorial-shots ARGS="--force"` 1 回で済みます。

**上げるときに一緒に決めること**（先に決めずに素材だけ大きくすると、鋭くならずに絵が
大きくなるだけになります）:

- **表示幅を止める**。素材が 1280 になると、`max-width: 100%` の下では画像が段いっぱい
  （website 688〜951px / GitHub 1012px）まで広がり、倍率は 1.26〜1.86x にしかなりません。
  website は CSS（`max-width: min(100%, 640px)`）で止められますが、GitHub の Markdown に
  CSS は効かないので、画像行を `<img width="640">` に変える是非まで含めて決めます
- **動きの証跡の上限**。`motion.json` の `width` は既定 720 で、いまは素材が 640 しか
  無いため実質 640 で頭打ちです。素材が大きくなると本当に 720 以上で書けるようになり、
  1 ファイル 500KB の上限（後述）に当たります。静止画は PNG 35.7KB → 85.3KB、
  WebP(q80) 8.1KB → 18.4KB でおよそ 2.3 倍（実測。平坦な絵なので 4 倍にはなりません）

DocC リファレンス（`docs/reference/`）は同じ問題を持ちますが程度が軽く、`@Row` の左列が
293px で頭打ちのところへ 480px の素材なので **1.64x** です。こちらも #1033 待ちです。
