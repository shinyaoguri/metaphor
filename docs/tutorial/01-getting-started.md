...
前提とするのは Swift の基本文法（型・クラス・関数・ループ）だけで、グラフィックスプログラミングの経験は要りません。動かすには macOS 14.0 以降の Apple Silicon の Mac が必要です（1.1）。
...
- [ ] metaphor が「毎フレーム 1 枚の絵を描き直す」フレームベースのライブラリだと分かった
- [ ] 動く環境が macOS 14.0 以降・Apple Silicon に限られると分かった
- [ ] チュートリアル・API リファレンス・Examples がそれぞれ何のためのものか分かった
...
- [ ] `metaphor new` と `metaphor run` でスケッチを作って動かせるようになった
- [ ] `@main` / `Sketch` / `config` / `draw()` がそれぞれ何を受け持つか分かった
- [ ] `background()` / `fill()` / `noStroke()` / `circle()` で図形を 1 つ描けるようになった
- [ ] 座標を `width` / `height` から作ると、キャンバスの大きさを変えても構図が保てると分かった
...
- [ ] `config` / `setup()` / `draw()` が呼ばれるタイミングの違いが分かった
- [ ] フレームをまたいで持ち越したい値は型のプロパティにする、と覚えた
- [ ] 色や線の太さといったスタイルが、変えるまで以降の描画すべてに残ると分かった
- [ ] `background()` を `draw()` で呼ばないと前のフレームの絵が残ると分かった
...
- [ ] 原点 `(0, 0)` が左上で、`y` が下へ向かって増えると分かった
- [ ] キャンバスの中心を `width * 0.5` / `height * 0.5` で表せるようになった
- [ ] レンダリング解像度とウィンドウの大きさが別物で、`windowScale` が両者をつなぐと分かった
- [ ] 縦横比が合わないときは絵が歪まず余白（レターボックス）が入ると分かった
...
- [ ] `metaphor watch` で保存のたびに絵が差し替わる状態を作れるようになった
- [ ] ビルドが通らない間は前の絵が残り、端末にエラーが出ると分かった
- [ ] 再ビルドをまたぐとプロパティが初期値に戻り、`preserveClock` で時計だけ引き継げると分かった
...
- [ ] `noLoop()` / `loop()` / `redraw()` の 3 つを使い分けられるようになった
- [ ] `frameRate(_:)` が指定するのは上限で、重ければ届かないと分かった
- [ ] 動かない絵は `setup()` で `noLoop()` を呼ぶと毎回同じ結果になると分かった
