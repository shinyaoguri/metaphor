...
> **2026-08-02 追記（#327）**: この「据え置き」は暫定ではなく **1.0 の確定仕様**になった（既定 ON 化は見送り）。下の Amendment を参照。

## Amendment（2026-08-02, Issue #327）— 既定 ON 化は見送り、現行 opt-in を確定仕様とする

**結論**: **見送り**。「影オン = 常時記録／影オフ = `METAPHOR_COMMAND_RECORD` opt-in」を **1.0 の確定仕様として凍結**する（下記 Decision）。ADR-0003 が Follow-ups に残した「既定 ON 化の判断（安定後）」はここで決着する。
...
**Option B を採用する**（2026-08-02 のユーザーレビューで確定）。決め手は「既定 ON で新たに得られるものが『影オフでも 2D を 3D の背後に置ける』**だけ**であり、それは既に opt-in で得られる」のに対し、「代償の `loadPixels()` 非互換は全利用者に及び、同じ v0.8.x ウィンドウで達成した W1-7 を打ち消す」という非対称である。実測 CPU コスト（+0.1〜0.5 ms/frame）は単体なら許容範囲だが、機能上の利得がゼロである以上、それを全利用者に課す理由がない。
...
#### 2026-08-16 追記（#682）— Decision 3 の保証が成り立つ条件

Decision 3 の文面は**無条件に読める**が、実際には条件がある。即時経路の重ね順は「呼び出し順」ではなく **エンコードされた順** で決まり、2D はバッチが途中で吐かれた時点でそこまでの描画が確定してしまう。記録経路は `Canvas3D.flushPending2D` が 3D 記録の直前に 2D を確定する（「設計上の発見」節）ため呼び出し順をそのまま復元する。したがって **フレーム末尾より前に吐かれた 2D は、即時経路でだけ 3D の背後へ回る**。

保証が成り立つ条件は「2D を 3D より先に描かないこと」ではなく、正確には:
...
| 契機 | 実装 |
|---|---|
| `blendMode()` の変更 | `Canvas2D.blendMode(_:)`（3 種のバッチをすべて flush） |
...
支配的パターン `background → 3D → オーバーレイ` はオーバーレイの後に何も描かないので条件を自然に満たす。踏むのは「3D の前面に置いた 2D の **後** に、別種の 2D を描いたとき」で、利用者から見ると **「テキストを 1 行足しただけで帯が箱の後ろに回る」** という形で現れる。PR #681（#375 の常設テスト化）は実際にこれを踏んでいる（半透明帯 → 半透明円の順に描いたところ、円のパイプライン切り替えが帯を先に吐き出した）。

もう 1 点、Decision 3 は記録経路の 2 つの入口（影オン / `METAPHOR_COMMAND_RECORD` opt-in）をひとまとめに書いているが、§2 の実測と #375 の常設テスト（`Tests/metaphorTests/CommandRecordParityTests.swift`）が即時経路と突き合わせているのは **opt-in 側だけ**である。影オンは影そのものが絵を変えるため「一致するはず」と言えるのが影の可視でないシーンに限られ、テストとして安定しない。

この非対称そのものは 1.0 の仕様として残す（ADR-0002 / [deterministic-rendering.md](../design/deterministic-rendering.md) の既知の制限）。本追記は文面に条件を補うもので、**Decision 3 自体は変更しない**。
...
- 別 Issue とするもの:
  ...
  - `METAPHOR_COMMAND_RECORD` を `DEVELOPMENT.md` の環境変数表へ追記（Decision 2 で 1.0 維持と決めたのに未文書）→ **#376 に起票済み**。
