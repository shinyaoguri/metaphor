...
スニペットのスケッチは 480x360 で撮っています。`@Row` の左列は**広い画面でも 293px で
頭打ち**なので（実測 293.3px。列が縦に折れる 735px 以下では本文幅いっぱいまで広がります）、
dpr 2 の画面に対しては **1.64x** です。Retina に必要な 2x には届きませんが、チュートリアルの
1.00x よりは軽い不足です（[#1025](https://github.com/shinyaoguri/metaphor/issues/1025) の実測）。

外部 URL では `@2x` の出し分けが効かないため、上げるなら素材そのものを大きくすることに
なります。構図を保ったまま画素だけ増やす手段がライブラリに無いので
（[#1033](https://github.com/shinyaoguri/metaphor/issues/1033)）、それが入るまで 480 のままです。
...
- `symmetric` — `vertical`（上下反転しても同じでよい）/ `horizontal`（左右）/ `both`。
  下の「反転一致の警告」を軸ごとに黙らせます
...
**待ち時間の指定はありません**。リクエストは静止画も動きも起動前に置き、常に frame 1 から
撮ります（[#784](https://github.com/shinyaoguri/metaphor/issues/784)）。撮り始めを秒で遅らせる
と、待つあいだに進むフレーム数が実行ごとに変わり、撮り直すたびに別の絵になります。
「数フレーム走らせてから」が要るスニペットは、`frameCount` から位相をスニペット側で作って
ください。
...
4. 撮った絵が上下・左右反転と見分けが付かないかを調べ、付かなければ警告する（下記）
5. Gyazo へ上げ、台帳 `images/manifest.json` と doc コメントの画像行へ書き戻す
...
撮った直後に、その PNG を**上下反転・左右反転したものとの PSNR** を測り、
`FLIP_PSNR_WARN_DB`（40 dB）を超えたら警告します。反転しても見分けが付かない絵は、
**向きを決める引数を間違えても同じ絵になる**ということで、実行結果が引数の誤りを写せて
いません。[#923](https://github.com/shinyaoguri/metaphor/issues/923) の `ortho` /
`perspective` がそれでした。

この穴は**絵を見比べても発見できません**（気付いたきっかけは「撮り直したのに sha256 と
Gyazo URL が完全に同一だった」という副次的な観察）。だから機械で拾います。

**エラーにはしません。** 対称なのが正しい絵（`radialGradient`、真円、正方形）は普通に
あり、撮影を止めると意図して対称な絵を出したいときに作業が詰まります。分かっているものは
`shots.config.json` の `"symmetric"` で軸ごとに黙らせてください。

しきい値は台帳の 58 点を実測して決めました（Gyazo の URL から落として sha256 照合。撮影は
決定的なので本番と同じ値 — [#586](https://github.com/shinyaoguri/metaphor/issues/586)）。

| | vflip | hflip |
|---|---|---|
| `ortho` / `perspective`（#923 で直す前） | inf / 58.7 dB | — |
| 40 dB 超（実測で 6 点） | 98.8 / 65.6 / 43.5 dB | 94.0 / 66.3 / 65.6 / 49.4 / 45.2 / 43.5 dB |
| 次に高い点（`strokeWeight`） | — | 37.0 dB |
| `ortho` / `perspective`（直した後） | 16.7 / 20.5 dB | 17.9 / 17.6 dB |

40 dB は 37.0 dB と 43.5 dB のあいだの空白帯に落ちます。真円や正方形を中央に描いた絵でも
ラスタライズの半画素ずれで 33〜35 dB にしかならないので、**「幾何的に対称」ではなく
「画素の水準で反転と見分けが付かない」だけ**を拾えています。

`ffmpeg` は GIF 生成で既に要るので追加の依存はありません（無い環境では黙って飛ばします）。
...
ライティング・画像リソースを読むもの、いずれも一致。1 本あたり 1〜2 秒）。

**動きも一致します**（[#784](https://github.com/shinyaoguri/metaphor/issues/784)）。以前は
下見を 1 枚撮って `settle` 秒待ってから本番のリクエストを置いていたため、待つあいだに進む
フレーム数が実行ごとに変わり、`frameCount` で駆動するスニペットは毎回違う位相から撮れて
いました。いまは静止画と同じく起動前に置くので frame 1 から撮り始めます（`rotate(_:)` /
`rotateY(_:)` / `ease(_:from:to:using:)` を 2 回ずつ撮り、代表静止画と GIF の両方で sha256 が
一致することを確認）。ただし**現在公開されている 3 本の GIF は #784 より前に撮ったもの**
なので、初回の撮り直しでは新しいアセットが 1 つずつ増えます（以後は安定します）。
