...
extension Canvas2D {
    ...
    /// 得られます。字送りも `text()` と同じで、``textWidth(_:)`` の advance を
    /// 1 文字ずつ積みます（`textWidth("ab") == textWidth("a") + textWidth("b")` が成り立ち、
    /// 語ごとに測って組めます）。
    ///
    /// - Note: 1 文字ずつ置くので、**複数文字にまたがるシェーピングは掛かりません**
    ///   — リガチャ（`fi` の合字など）、アラビア文字の連結形、インド系文字の並べ替えは
    ///   反映されず、カーニングも入りません。`text()` の描画と揃えるための選択です
    ///   （Processing も同じくカーニングを掛けません）。フォントに無い文字の
    ///   フォールバックは従来どおり効きます。
    ///
