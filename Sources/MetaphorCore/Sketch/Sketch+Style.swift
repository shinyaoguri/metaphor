...
extension Sketch {
    ...
    ///
    /// - Note: **2D と 3D の両方**に作用します（``fill(_:)`` / ``stroke(_:)`` の
    ...
    ///   ``stroke(_:)`` を呼んでいなければ見た目は変わりません（ADR-0005）。
    ///
    /// ### 実行結果
    ///
    ...
    ///   3D の ``texture(_:)`` には効きません——3D のテクスチャは ``fill(_:)`` の色を
    ///   乗算して着色するため、ティント専用のスロットがそもそもありません（ADR-0005）。
    ///   テクスチャ付きの 3D に色を掛けたいときは ``fill(_:)`` を使ってください。
    ///
    ...
    ///
    /// - Note: **2D のみ**に作用します（3D のテクスチャは ``fill(_:)`` の色で着色します。
    ///   ADR-0005）。
    ///
    /// - Parameters:
    ///   - v1: 第1カラーチャンネル値（赤または色相）。
    ...
    /// 画像のティントを解除します。
    ///
    /// - Note: **2D のみ**に作用します（``tint(_:)`` と対。ADR-0005）。
    public func noTint() {
        context.noTint()
    }
}
