...
/// - `.lines` / `.points` はインデックスが空なので何も起きない。
///
/// - Note: イミディエイト側（`Canvas3D+Shapes.swift` の `autoComputeNormals()` と
///   `drawShape3DPolygon()`）は「3 頂点ごと」の別規則のままで、まだここへ寄せていません。
///   そちらはストリップ／ファンで後ろの頂点が既定値に取り残される穴を抱えており、
///   移行すると絵が変わるため別 Issue（#875）で扱います。
enum FaceNormals {
