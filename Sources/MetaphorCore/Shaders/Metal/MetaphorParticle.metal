...
struct ParticleUniforms {
    ...
    uint  desiredEmit;   // number of particles to emit this frame (CPU accumulates emissionRate*dt)
    uint  _pad3;
    uint  _pad4;
...
kernel void metaphor_particleUpdate(
    ...
    device atomic_uint *emitCounter [[buffer(4)]],
    ...
    if (p.sizeAndFlags.w < 0.5) {
        // Assign each dead slot an ordinal via atomic counter and spawn exactly
        // desiredEmit particles this frame. The old probabilistic formula
        // (emissionRate * dt / particleCount) was applied to dead slots only,
        // so the effective emission rate decayed as the pool filled up.
        uint slot = atomic_fetch_add_explicit(emitCounter, 1u, memory_order_relaxed);
        uint timeSeed = uint(uniforms.time * 1000.0);

        if (slot < uniforms.desiredEmit) {
