...
///
/// ``Tween`` は経過時間から求めた進行率（0.0...1.0 に収まる）にイージング関数を適用し、
/// **その結果**を `t` として渡します。`easeOutBack` / `easeOutElastic` のように
/// オーバーシュートするイージングは 1.0 を超えたり 0.0 を下回ったりするため、
/// `t` は 0.0...1.0 の範囲外になり得ます。
///
/// 組み込みの `Float` / `SIMD2` / `SIMD3` / `SIMD4` / ``Color`` はいずれも `t` を
/// クランプせず線形外挿します。独自に準拠する型も外挿を許容してください
/// （`t` をクランプすると、同じイージングでも組み込み型と見た目が変わります）。
public protocol Interpolatable {
    /// パラメータ t で2つの値を線形補間します。
    ///
    /// - Parameters:
    ///   - from: 開始値。
    ///   - to: 終了値。
    ///   - t: 補間係数。通常は 0.0...1.0 ですが、オーバーシュートするイージングでは
    ///     この範囲を外れます。クランプせず外挿してください。
    /// - Returns: 補間された値。`t` が範囲外なら `from`〜`to` の外側の値。
