...
外部への出力（ライブビューアへのフレーム転送、別パッケージ [metaphor-syphon](https://github.com/shinyaoguri/metaphor-syphon) の Syphon 出力など）は、ブリットの前に出力プラグイン（``MetaphorCore/MetaphorOutputPlugin``）が同じオフスクリーンテクスチャを受け取ります。

このアーキテクチャにより以下が可能になります:

- **解像度の独立性** — ウィンドウサイズに関係なく 1920×1080 でレンダリング。
- **外部出力** — 固定解像度でレンダリングしたフレームをライブビューアや VJ ソフトウェア（Syphon、metaphor-syphon プラグイン）に送信。
...
5. **出力フェーズ** — 登録された出力プラグインの `post()` が走り、フレームがライブビューアや Syphon（metaphor-syphon）へ発行。
